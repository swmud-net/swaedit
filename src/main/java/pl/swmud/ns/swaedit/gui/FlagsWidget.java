...
public class FlagsWidget extends QWidget {
	...
	public FlagsWidget(List<Flag> flagList, FlagsWrapper flagsWrapper, String flagsName, boolean allowNone) {
		ui.setupUi(this);
		...
		flagBox = new QCheckBox[this.flagList.size()];
		int i = 0, j = 0, k = 0;
		QGridLayout gl = new QGridLayout(ui.flagGroupBox);
		for (Flag flag : this.flagList) {
			flagBox[k] = new QCheckBox(flag.getName());
			flagBox[k]
			        .setChecked((flagsValue.longValue() & flag.getValue().longValue()) == flag.getValue().longValue());
	...
	public FlagsWidget(List<Flag> flagsList, FlagsWrapper flagsWrapper, String flagsName) {
		this(flagsList, flagsWrapper, flagsName, false);
	}
