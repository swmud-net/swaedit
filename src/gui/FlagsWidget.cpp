    ...
    , initialValue_(initialValue)
{
    ...
    for (int i = 0; i < flagList_.size(); ++i) {
        ...
        bool checked = (flag.value == 0) ? (flagsValue_ == 0)
                                        : ((flagsValue_ & flag.value) == flag.value);
...
void FlagsWidget::onCheckBoxStateChanged(int state)
{
    ...
    if (flagValue == 0) {
        // NONE: checking it clears all flags
        if (state == Qt::Checked)
            flagsValue_ = 0;
    } else {
        if (state == Qt::Checked)
...
void FlagsWidget::updateCheckboxesFromValue()
{
    for (QCheckBox *cb : checkboxes_) {
        ...
        bool checked = (flagValue == 0) ? (flagsValue_ == 0)
                                        : ((flagsValue_ & flagValue) == flagValue);
        cb->setChecked(checked);
    }
}
